<x-app-layout>
    <x-slot name="header">
        <a href="{{ route('assessments') }}" class="float-right flex text-indigo-700">
        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Upload Result - ').$assessment['title'] }}
        </h2>
    </x-slot>
                ...
                <div x-data="{ open: false, working: false }" x-cloak>
                    ...
                    <div class="p-3 sm:px-5 bg-white border-b border-gray-200">
                        <div class="md:grid md:grid-cols-3 md:gap-6">
                            <x-jet-section-title>
                                <x-slot name="title">{{ __('Upload Assessment Results') }}</x-slot>
                                <x-slot name="description">
                                    {{ __('Ensure the file to be uploaded correlates with the template.') }}
                                    You can download the template <a href="{{ route('assessments.template') }}" target="_blank" class="underline text-purple-700">Here</a>
                                </x-slot>
                            </x-jet-section-title>

                            <div class="mt-5 md:mt-0 md:col-span-2">
                                <form method="POST" enctype="multipart/form-data" wire:submit.prevent="{{ route('assessments.process') }}">
                                    @csrf
                                    <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                                        <div class="grid grid-cols-6 gap-6">
                                            <div class="col-span-6 sm:col-span-4">
                                                <label for="">{{ __('Assessment*') }}</label>
                                                <x-jet-input type="text" readonly value="{{ $assessment['title'] }}" class="mt-1 block w-full" />
                                            </div>
                                            <div class="col-span-6 sm:col-span-4">
