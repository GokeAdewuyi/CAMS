        ...
        @foreach($courses as $key => $course)
            <div class="p-6 border-t border-gray-200 md:border-t-0 md:border-l">
                <div class="flex items-center">
                    ...
                    <div class="ml-4 text-lg text-gray-600 leading-7 font-semibold"><a href="{{ route('students').'?c='.$course['id'] }}">{{ $course['code'] }}</a></div>
                </div>

                <div class="ml-12">
                    <div class="mt-2 text-sm text-gray-500">{{ $course['title'] }}</div>

                    <a href="{{ route('students').'?c='.$course['id'] }}">
                        <div class="mt-3 flex items-center text-sm font-semibold text-indigo-700">
                            <div>Add / View Enrolled Students</div>
